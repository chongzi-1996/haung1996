...
public class addgood extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		good.setGkind(gkind);
		int row=goodimpl.addGood(good);
		if(row==1){
			out.println("<script>alert('添加成功');window.location.href='admin/manager/addGood.jsp'</script>");	
		}else{
			out.println("<script>alert('添加失败');window.location.href='admin/manager/addGood.jsp'</script>");	
		}
		}
